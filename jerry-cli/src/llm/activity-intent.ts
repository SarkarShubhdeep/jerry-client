...
export type ActivityTimeRange = {
  ...
  /** Shown in AW context and frontmatter so the model knows the window. */
  label: string
}
...
/** True when the user means the previous calendar day (local time). */
...
/** User explicitly wants the full stored ActivityWatch timeline. */
...
/** Use AW bucket metadata when the user asks for full history. */
...
/**
 * Parse a time window from the user prompt, or null if none was given.
 */
